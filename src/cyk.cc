...
Statement::Var_Decl *get_tile_computation_outside(
  ...
  return new Statement::Var_Decl(
      new Type::Int, VARNAME_num_tiles_per_axis,
      new Expr::Cond(
...
enum CYKmode {
  ...
  OPENMP_PARALLEL_OUTSIDE,
  OPENMP_SERIAL_OUTSIDE
};

CYKloop get_for_column(Expr::Vacc *running_boundary, Expr::Base *start,
  ...
  if (with_checkpoint && (mode != CYKmode::OPENMP_PARALLEL)) {
    ...
    start = new Expr::Cond(new Expr::Vacc(new std::string(
                               *running_boundary->name() + "_loaded++")),
                           start, running_boundary);
  }

  Statement::Var_Decl *var_col =
      new Statement::Var_Decl(t, running_boundary, start);

  // create condition of For loop
  Expr::Base *cond_col = new Expr::Less(new Expr::Vacc(*var_col), end);
  if (mode == CYKmode::OPENMP_SERIAL_OUTSIDE) {
    cond_col = new Expr::Greater(
        (new Expr::Vacc(*var_col))->plus(new Expr::Const(1)), end);
  }
...
Statement::For *get_for_openMP(Expr::Vacc *loopvar, Expr::Base *start,
  ...
  Statement::Var_Decl *var =
      new Statement::Var_Decl(new Type::Int(), loopvar, start);
...
std::list<Statement::Base *> *cyk_traversal_singlethread_singletrack(
  ...
  Expr::Base *row_start =
      ast.grammar()->right_running_indices.at(track)->plus(new Expr::Const(1));
  ...
  CYKloop row =
      get_for_row(ast.grammar()->left_running_indices[track], row_start,
                  new Expr::Const(1), with_checkpoint, mode);
  row.loop->statements.insert(row.loop->statements.end(), co->begin(),
                              co->end());
  ...
  col.loop->statements.insert(col.loop->statements.end(), co->begin(),
                              co->end());
  ...
  CYKloop rowC =
      get_for_row(ast.grammar()->left_running_indices[track], row_start,
                  new Expr::Const(1), with_checkpoint, mode);
  co = copy_statements(nested_stmts);
  rowC.loop->statements.insert(rowC.loop->statements.end(), co->begin(),
                               co->end());
...
std::list<Statement::Base *> *cyk_traversal_singlethread_singletrack_outside(
  ...
  Expr::Vacc *idx_col = new Expr::Vacc(
      new std::string(*ast.grammar()->right_running_indices[track]->name() +
                      OUTSIDE_IDX_SUFFIX));
  Expr::Vacc *idx_row = new Expr::Vacc(
      new std::string(*ast.grammar()->left_running_indices[track]->name() +
                      OUTSIDE_IDX_SUFFIX));

  Expr::Base *col_start = seqend->minus(idx_row);
  Expr::Base *col_end = seqend->plus(new Expr::Const(1));
  CYKloop col =
      get_for_column(idx_col, col_start, col_end, with_checkpoint, mode);
  std::list<Statement::Base *> *co = copy_statements(nested_stmts);
  col.loop->statements.insert(col.loop->statements.end(), co->begin(),
                              co->end());
  ...
  CYKloop row =
      get_for_row(idx_row, row_start, seqend->plus(new Expr::Const(1)),
...
std::vector<Statement::Base *> *get_wait_omp(std::string var_outer,
                                             std::string var_inner,
                                             Expr::Base *step_size,
                                             std::string *z,
                                             bool no_inner_loop) {
  std::vector<Statement::Base *> *stmts = new std::vector<Statement::Base *>();

  stmts->push_back(new Statement::CustomCode("#pragma omp ordered"));
  Statement::Block *blk_omp2 = new Statement::Block();
  blk_omp2->statements.push_back(new Statement::CustomCode(
      "// force omp to wait for all threads to finish "
  ...
  if (!no_inner_loop) {
    blk_omp2->statements.push_back(new Statement::Var_Assign(
        ...
        (new Expr::Vacc(new std::string(var_inner)))->plus(step_size)));
  }
  blk_omp2->statements.push_back(new Statement::Var_Assign(
 ...
 *
 */
std::list<Statement::Base *> *cyk_traversal_multithread_parallel(
    const AST &ast, Statement::Var_Decl *seq, std::string *tile_size,
  ...
  CYKloop col = get_for_column(idx_col, z, z->plus(new Expr::Vacc(tile_size)),
                               with_checkpoint, mode);
  col.loop->statements.push_back(row.loop);
  Expr::Base *start_z = new Expr::Const(0);
  if (with_checkpoint) {
    start_z = new Expr::Vacc(new std::string(std::string(var_ol1) + "_start"));
  }
  Statement::For *loop_z = get_for_openMP(
      z, start_z, new Expr::Vacc(name_maxtilen), &tile_size_decl);
  if (with_checkpoint) {
  ...
  if (with_checkpoint) {
    loop_z->statements.push_back(
        new Statement::CustomCode("#pragma omp ordered"));
    ...
    blk_omp->statements.push_back(new Statement::CustomCode(
        ...
        "(of size tile_size)"));
    blk_omp->statements.push_back(
        new Statement::Var_Assign(new Var_Acc::Plain(new std::string(var_ol1)),
  ...
  CYKloop rowB =
      get_for_row(idx_row, new Expr::Vacc(*x),
                  (new Expr::Vacc(*x))->minus(new Expr::Vacc(tile_size)),
                  with_checkpoint, mode);
  CYKloop colB = get_for_column(idx_col, y, y->plus(new Expr::Vacc(tile_size)),
                                with_checkpoint, mode);
  ...
  if (with_checkpoint) {
    start_y = new Expr::Cond(
        new Expr::Vacc(new std::string(std::string(var_il2) + "_loaded")), z,
        new Expr::Vacc(new std::string(std::string(var_il2) + "_start")));
  }
  Statement::For *loop_y = get_for_openMP(
      y, start_y, new Expr::Vacc(name_maxtilen), &tile_size_decl);
  // produce: unsigned int x = y - z + tile_size;
  if (with_checkpoint) {
    loop_y->statements.push_back(
        new Statement::CustomCode("++inner_loop_2_idx_loaded;"));
    loop_y->statements.push_back(mutex_lock());
  }
  loop_y->statements.push_back(x);
  loop_y->statements.push_back(colB.loop);
  if (with_checkpoint) {
    std::vector<Statement::Base *> *omp_wait = get_wait_omp(
        var_ol2, var_il2, new Expr::Vacc(tile_size), z->name(), false);
    loop_y->statements.insert(loop_y->statements.end(), omp_wait->begin(),
                              omp_wait->end());
  }

  Expr::Vacc *start_z2 = new Expr::Vacc(tile_size);
  if (with_checkpoint) {
    start_z2 = new Expr::Vacc(new std::string(std::string(var_ol2) + "_start"));
  }
  loop_z = get_for_openMP(z, start_z2, new Expr::Vacc(name_maxtilen),
                          &tile_size_decl);
  if (with_checkpoint) {
    loop_z->statements.push_back(
        new Statement::CustomCode("#pragma omp for ordered schedule(dynamic)"));
  } else {
    loop_z->statements.push_back(new Statement::CustomCode("#pragma omp for"));
  }
  loop_z->statements.push_back(loop_y);
  if (with_checkpoint) {
    loop_z->statements.push_back(new Statement::Var_Assign(
        new Var_Acc::Plain(new std::string(var_il2)), z));
  }
...
Statement::For *get_triag_traversal(
  ...
  if (cp == OMP_OUTSIDE_CP::A) {
    ...
    start_j =
        new Expr::Cond(new Expr::Vacc(new std::string(name_col + "_loaded++")),
                       start_j, new Expr::Vacc(new std::string(name_col)));
  }
  Statement::Var_Decl *lv_j =
      new Statement::Var_Decl(type_diag, name_col, start_j);
  // short cut for t_x_j for variable access
  Expr::Vacc *j = new Expr::Vacc(*lv_j);

  Expr::Less *cond_j = new Expr::Less(
      j,
      new Expr::Cond(new Expr::Less(diag, tl), tl,
  ...
  fl_j->push_back(new Statement::Var_Decl(new Type::Size(), name_row,
                                          diag->plus(j)->minus(tl)));
  if (nested_stmts) {
    fl_j->statements.insert(fl_j->statements.end(), nested_stmts->begin(),
                            nested_stmts->end());
  }
  if (cp == OMP_OUTSIDE_CP::A) {
    ...
    fl_j->statements.insert(fl_j->statements.end(), omp_wait->begin(),
                            omp_wait->end());
  }
  ...
  if (cp == OMP_OUTSIDE_CP::A) {
    fl_diag->statements.push_back(new Statement::Var_Assign(
        new Var_Acc::Plain(new std::string(std::string(VARNAME_InnerLoop2) +
                                           OUTSIDE_IDX_SUFFIX)),
 ...
 *
 */
std::list<Statement::Base *> *cyk_traversal_multithread_outside(
    const AST &ast, Expr::Base *seqsize, std::string *tile_size,
  ...
  if (part == "A") {
    ...
    for_tile->push_back(get_triag_traversal(
        ...
        std::string(*(ast.grammar()->left_running_indices[track])->name() +
                    OUTSIDE_IDX_SUFFIX),
        true, false, nullptr, new Expr::Const(1), OMP_OUTSIDE_CP::no));

    stmts->push_back(get_triag_traversal(
        std::string("matrix_diag_" + std::to_string(track)),
        new Expr::Vacc(*max_tiles), "y", "x", false, true, for_tile,
        new Expr::Const(1),
  ...
  if (part == "B") {
    ...
    Statement::For *for_tile_b = get_triag_traversal(
        ...
        std::string(*(ast.grammar()->left_running_indices[track])->name() +
                    OUTSIDE_IDX_SUFFIX),
        false, false, nullptr, new Expr::Const(1),
    ...
    if (with_checkpoint) {
      pragma += " ordered schedule(dynamic)";
      diag_start = new Expr::Vacc(new std::string(
          std::string(VARNAME_OuterLoop1) + OUTSIDE_IDX_SUFFIX));
    }
    ...
    if (with_checkpoint) {
      ...
      fl_diag->statements.insert(fl_diag->statements.end(), omp_wait->begin(),
                                 omp_wait->end());
    }
  ...
  if (part == "C") {
    ...
    Expr::Base *start_diag = new Expr::Cond(
        ...
        (new Expr::Times(new Expr::Vacc(tile_size),
                         (new Expr::Vacc(&VARNAME_num_tiles_per_axis))
                             ->plus(new Expr::Const(1))))
            ->plus(new Expr::Const(1)),
        new Expr::Const(1));
    ...
    if (with_checkpoint) {
      start_diag = new Expr::Cond(
          ...
          start_diag, new Expr::Vacc(new std::string(diag_name)));
    }
    Statement::For *for_tile_c = get_triag_traversal(
        diag_name, seqsize->plus(new Expr::Const(1)),
        std::string(*(ast.grammar()->right_running_indices[track])->name() +
                    OUTSIDE_IDX_SUFFIX),
        std::string(*(ast.grammar()->left_running_indices[track])->name() +
                    OUTSIDE_IDX_SUFFIX),
        false, false, nullptr, start_diag,
...
size_t count_nt_calls_and_loops(Statement::For *loop) {
  ...
  for (std::list<Statement::Base *>::const_iterator i =
    ...
    if ((*i)->is(Statement::FN_CALL) &&
        (dynamic_cast<Statement::Fn_Call *>(*i)->name().find("nt_tabulate_",
                                                             0) == 0)) {
      res++;
    }
...
std::list<Statement::Base *> *add_nt_calls(std::list<Statement::Base *> &stmts,
                                           std::list<std::string *> *loop_vars,
                                           std::list<Symbol::NT *> orderedNTs,
  ...
  for (std::list<Statement::Base *>::iterator s = stmts.begin();
    ...
    if ((*s)->is(Statement::FOR)) {
      ...
      next_loop_vars->insert(next_loop_vars->end(), loop_vars->begin(),
                             loop_vars->end());
      if (((mode != CYKmode::OPENMP_PARALLEL) &&
      ...
      std::list<Statement::Base *> *new_stmts =
          add_nt_calls(fl->statements, next_loop_vars, orderedNTs,
                       with_checkpoint, mode, ast);
      fl->statements.insert(fl->statements.end(), new_stmts->begin(),
                            new_stmts->end());
    }
  }
    ...
    for (size_t t = 0; t < (*i)->tracks(); ++t, ++it_stmt_seq) {
      if (!(*i)->tables()[t].delete_left_index()) {
        ...
        if ((mode == CYKmode::SINGLETHREAD_OUTSIDE) ||
            ...
            (mode == CYKmode::OPENMP_SERIAL_OUTSIDE)) {
          idx = new Expr::Vacc(
              new std::string(*idx->name() + OUTSIDE_IDX_SUFFIX));
        }
        ...
        if (mode == CYKmode::SINGLETHREAD_OUTSIDE) {
          ...
          args->push_back(
              idx->plus((new Expr::Vacc(new std::string(
                            *(*i)->right_indices.at(t)->vacc()->name() +
                            OUTSIDE_IDX_SUFFIX))))
                  ->minus(seqsize));
          ...
          args->push_back(
              idx_i->plus(new Expr::Times(new Expr::Vacc(new std::string("x")),
                                          new Expr::Vacc(&VARNAME_tile_size))));
      ...
      if (!(*i)->tables()[t].delete_right_index()) {
        ...
        if ((mode == CYKmode::OPENMP_PARALLEL_OUTSIDE)) {
          ...
          args->push_back(
              idx_j
                  ->plus(new Expr::Times(new Expr::Vacc(new std::string("y")),
                                         new Expr::Vacc(&VARNAME_tile_size)))
                  ->plus(seqsize
                             ->minus(new Expr::Times(
                                 new Expr::Vacc(&VARNAME_num_tiles_per_axis),
                                 new Expr::Vacc(&VARNAME_tile_size)))
                             ->plus(new Expr::Const(1))));
        } else if ((mode == CYKmode::OPENMP_SERIAL_OUTSIDE)) {
...
Fn_Def *print_CYK(const AST &ast) {
    ...
    for (size_t track = 0; track < ast.grammar()->axiom->tracks(); ++track) {
      ...
      for (int io = 0; io < 2; ++io) {  // iterate through inside and outside
        fn_cyk->stmts.push_back(new Statement::Var_Decl(
            new Type::Int(), *(idx_i->name()) + suffix,
            new Expr::Or(new Expr::Not(new Expr::Vacc(
                             new std::string("load_checkpoint"))),
                         new Expr::Not(idx_i))));

        fn_cyk->stmts.push_back(new Statement::Var_Decl(
            new Type::Int(), *(idx_j->name()) + suffix,
            new Expr::Or(new Expr::Not(new Expr::Vacc(
                             new std::string("load_checkpoint"))),
                         new Expr::Not(idx_j))));
          ...
          if (io == 0) {
            ...
            fn_cyk->stmts.push_back(new Statement::Var_Decl(
                ...
                new Expr::Or(new Expr::Not(new Expr::Vacc(
                                 new std::string("load_checkpoint"))),
                             new Expr::Not(
                                 new Expr::Vacc(new std::string(i_outside))))));
          }

          idx_i = new Expr::Vacc(
              new std::string(std::string("t_") + std::to_string(track) +
                              std::string("_diag") + OUTSIDE_IDX_SUFFIX));
          idx_j = new Expr::Vacc(
              new std::string(*idx_j->name() + OUTSIDE_IDX_SUFFIX));
        }
      }
    }
  }
  ...
  std::list<Statement::Base *> *stmts =
      cyk_traversal_singlethread(ast, CYKmode::SINGLETHREAD);
  ...
  if (ast.outside_generation()) {
    fn_cyk->stmts.push_back(
        new Statement::CustomCode("// ... now compute outside DP matrices"));
    fn_cyk->stmts.push_back(new Statement::CustomCode(
    ...
    std::list<Statement::Base *> *new_stmts = add_nt_calls(
        *stmts, new std::list<std::string *>(),
        ast.grammar()->topological_ord(), ast.checkpoint && ast.checkpoint->cyk,
        CYKmode::SINGLETHREAD_OUTSIDE, ast);
    stmts->insert(stmts->end(), new_stmts->begin(), new_stmts->end());
    fn_cyk->stmts.insert(fn_cyk->stmts.end(), stmts->begin(), stmts->end());
  }
  ...
  if (ast.grammar()->axiom->tracks() == 1) {
    ...
    if (ast.checkpoint && ast.checkpoint->cyk) {
      ...
      for (int io = 0; io < 2; ++io) {  // iterate through inside and outside
        ...
        fn_cyk->stmts.push_back(new Statement::CustomCode(
            ...
            ";"));
        fn_cyk->stmts.push_back(new Statement::CustomCode(
            "int " + std::string(VARNAME_OuterLoop1) + suffix + "_start = (" +
            VARNAME_OuterLoop1 + suffix +
            "_loaded) ? 0 : " + VARNAME_OuterLoop1 + suffix + ";"));
        fn_cyk->stmts.push_back(new Statement::CustomCode(
            "int " + std::string(VARNAME_OuterLoop2) + suffix + "_start = (" +
            VARNAME_OuterLoop2 + suffix + "_loaded) ? " + step + " : " +
    ...
    Statement::SYCL_Submit_Kernel *blk_sycl = new Statement::SYCL_Submit_Kernel(
        queue,
        new Statement::Var_Decl(new Type::External("sycl::handler&"), "cgh"));

    bool *test = new bool(true);
    blk_sycl->statements.push_back(
        new Statement::SYCL_Accessor_Decl(value, value, test, test));
    blk_sycl->statements.push_back(
        new Statement::CustomCode("DOES THIS WORK?"));
    // blk_sycl->statements.push_back(new
    // Statement::CustomCode(gapc::return_type));
    ...
    if (ast.checkpoint && ast.checkpoint->cyk) {
      ...
      blk_parallel->statements.push_back(
          new Statement::CustomCode("#pragma omp for"));
    }
    blk_parallel->statements.push_back(new Statement::CustomCode(
    ...
    if (ast.outside_generation()) {
      ...
      Statement::If *input_large_enough =
          new Statement::If(new Expr::Greater(seqsize, tl));
      input_large_enough->then.push_back(
          new Statement::CustomCode("#pragma omp parallel"));
      ...
      std::list<Statement::Base *> *stmts_outsideA =
          cyk_traversal_multithread_outside(
              ast, seqsize, &VARNAME_tile_size, max_tiles,
              ast.checkpoint && ast.checkpoint->cyk,
              CYKmode::OPENMP_PARALLEL_OUTSIDE, "A");
      // inject NT calls
      add_nt_calls(*stmts_outsideA, new std::list<std::string *>(),
                   ast.grammar()->topological_ord(),
                   ast.checkpoint && ast.checkpoint->cyk,
                   CYKmode::OPENMP_PARALLEL_OUTSIDE, ast);
      blk_parallel_outside->statements.insert(
          blk_parallel_outside->statements.end(), stmts_outsideA->begin(),
          stmts_outsideA->end());
      ...
      add_nt_calls(*stmts_outsideB, new std::list<std::string *>(),
                   ast.grammar()->topological_ord(),
                   ast.checkpoint && ast.checkpoint->cyk,
                   CYKmode::OPENMP_PARALLEL_OUTSIDE, ast);
      blk_parallel_outside->statements.insert(
          blk_parallel_outside->statements.end(), stmts_outsideB->begin(),
          stmts_outsideB->end());

      blk_parallel_outside->statements.push_back(
          new Statement::CustomCode("// end parallel"));
      ...
      fn_cyk->stmts.insert(fn_cyk->stmts.end(), stmts_outsideC->begin(),
                           stmts_outsideC->end());
      add_nt_calls(*stmts_outsideC, new std::list<std::string *>(),
                   ast.grammar()->topological_ord(),
                   ast.checkpoint && ast.checkpoint->cyk,
